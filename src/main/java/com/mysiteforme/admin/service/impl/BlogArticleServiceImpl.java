...
public class BlogArticleServiceImpl extends ServiceImpl<BlogArticleDao, BlogArticle> implements BlogArticleService {
    ...
    @Cacheable(value = "myarticle", key = "'directive_limit_'+#paramMap['limit'].toString()" +
            "+'_channelId_'+#paramMap['channelId'].toString()", unless = "#result == null or #result.size() == 0")
    @Override
    ...
    @Cacheable(value = "myarticle", key = "'directive_index_limit_'+#paramMap['limit'].toString()" +
            "+'_order_'+#paramMap['order'].toString()", unless = "#result == null or #result.size() == 0")
    @Override
    ...
    public void createArticlIndex() {
        ...
        for (BlogArticle blogArticle : list) {
            ...
            doc.add(new TextField("marks", blogArticle.getMarks() == null ? "" : blogArticle.getMarks(), Field.Store
                    .YES));
    ...
    @Cacheable(value = "myarticle", key = "'time_line_channel_id_'+#id.toString()", unless = "#result == null or " +
            "#result.size() == 0")
    @Override
    ...
    @Cacheable(value = "blogTagsData", key = "'tag_'+#map['articleId'].toString()" +
            "+'_sameArticles_limit_'+#map['limit'].toString()")
    @Override
    ...
    @Override
    public void saveOrUpdateTree(Long treePatentId) {
        blogArticleDao.saveOrUpdateTree(treePatentId);
    }
}
