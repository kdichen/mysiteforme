...
public class BlogChannelServiceImpl extends ServiceImpl<BlogChannelDao, BlogChannel> implements BlogChannelService {
    ...
    @Cacheable(value = "channelData", key = "'blog_channel_top_limit'+#limit", unless = "#result == null or #result" +
            ".size() == 0")
    @Override
    ...
    @Cacheable(value = "channelData", key = "'blog_parent_channel_list_'+#channelId", unless = "#result == null or " +
            "#result.size() == 0")
    @Override
    ...
    @Override
    public String selectPatentTreeId(String channelId) {
        return blogChannelDao.selectPatentTreeId(channelId);
